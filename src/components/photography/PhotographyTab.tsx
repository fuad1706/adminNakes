...
import PhotographyImageForm from "./PhotographyImageForm";
import type { FormValues } from "./PhotographyImageForm";
import PhotographyImageGrid from "./PhotographyImageGrid";
...
const PhotographyTab: React.FC<PhotographyTabProps> = ({
  ...
  const fetchImages = async () => {
    try {
      ...
      if (!token) {
        ...
        const publicResponse = await fetch(`${BASE_URL}/api/photography`);
      ...
      try {
        const response = await fetch(`${BASE_URL}/api/photography/admin`, {
          headers: {
    ...
    try {
      if (editingImage && editingImage._id) {
        ...
        const response = await fetch(
          `${BASE_URL}/api/photography/${editingImage._id}`,
          {
            method: "PUT",
            headers: {
              Authorization: `Bearer ${token}`,
        ...
        const response = await fetch(`${BASE_URL}/api/photography`, {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
            "x-auth-token": token,
  ...
  const handleDelete = async (imageId: string) => {
    ...
    try {
      const response = await fetch(`${BASE_URL}/api/photography/${imageId}`, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${token}`,
          "x-auth-token": token,
  ...
  const handleReorder = async (imageId: string, direction: "up" | "down") => {
    ...
    try {
      ...
      const response = await fetch(`${BASE_URL}/api/photography/reorder`, {
        method: "PUT",
        headers: {
      ...
      if (response.ok) {
        ...
        const update1 = fetch(`${BASE_URL}/api/photography/${img1._id}`, {
          method: "PUT",
          headers: {
            Authorization: `Bearer ${token}`,
            "x-auth-token": token,
        ...
        const update2 = fetch(`${BASE_URL}/api/photography/${img2._id}`, {
          method: "PUT",
          headers: {
            Authorization: `Bearer ${token}`,
            "x-auth-token": token,
  ...
  return (
    <>
      ...
      <PhotographyImageForm
        editingImage={editingImage}
        loading={loading}
        onSubmit={handleFormSubmit}
        onCancel={resetForm}
      />
      <PhotographyImageGrid
        images={images}
        onEdit={handleEdit}
